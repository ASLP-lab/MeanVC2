...
Usage (120ms_40ms):
  python src/infer/infer_e2e.py \
    ...
    --output-wav output.wav --chunk-size 12 --steps 3
...
Usage (40ms_40ms):
  python src/infer/infer_e2e.py \
    ...
    --output-wav output.wav --chunk-size 4 --steps 3
"""
...
def extract_bn(wav_path, asr_model, stride):
...
# ---- VC inference (from infer_zero_shot.py) ----
@torch.inference_mode()
def vc_inference(model, vocos, bn, spk_emb, chunk_size, steps, device):
    block_size = 4
    timesteps = torch.linspace(1.0, 0.0, steps + 1, device=device)
    bn_t = torch.from_numpy(bn).to(device).unsqueeze(0).transpose(1, 2)
    bn_t = torch.nn.functional.interpolate(bn_t, size=int(bn_t.shape[2] * 4),
    ...
    noise = torch.randn(seq_len, 80, device=device, dtype=bn_t.dtype).unsqueeze(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source-wav', type=str, required=True, help='Path to source audio')
    parser.add_argument('--target-wav', type=str, required=True, help='Path to target speaker audio')
    parser.add_argument('--model-config', type=str, required=True, help='VC model config JSON')
    parser.add_argument('--ckpt-path', type=str, required=True, help='VC model checkpoint')
    parser.add_argument('--vocoder-ckpt-path', type=str, default='ckpts/vocos/vocos.pt')
    parser.add_argument('--output-wav', type=str, default='output.wav', help='Output wav path')
    parser.add_argument('--output-mel', type=str, default=None, help='Output mel npy path')
    parser.add_argument('--asr-ckpt-path', type=str,
                        default=os.path.join(_ROOT, 'preprocess/ckpts/fastu2pp_80ms.pt'))
    parser.add_argument('--bn-stride', type=int, default=8,
                        help='BN stride: 8 for both 120ms and 40ms models')
    ...
    print(f"[1/5] Loading ASR model (JIT Fast-U2++)...")
    asr_model = load_asr_model(args.asr_ckpt_path, 'cpu')

    print(f"[2/5] Loading WavLM model (Spk Emb extraction)...")
    ...
    vc_model = load_checkpoint(vc_model, args.ckpt_path, device=device, use_ema=True)
    vc_model = vc_model.float().eval()
    vocos = torch.jit.load(args.vocoder_ckpt_path).to(device)

    print(f"[4/5] Extracting features (stride={args.bn_stride})...")
    t0 = time.time()
    bn = extract_bn(args.source_wav, asr_model, args.bn_stride)
    spk_emb = extract_spk_emb(args.target_wav, spk_model, device)
    print(f"   BN shape: {bn.shape}, SpkEmb shape: {spk_emb.shape}, time: {time.time()-t0:.1f}s")

    print(f"[5/5] VC inference...")
    t0 = time.time()
    mel, wav = vc_inference(vc_model, vocos, bn, spk_emb, args.chunk_size, args.steps, device)
    print(f"   Inference time: {time.time()-t0:.1f}s, output duration: {wav.shape[-1]/16000:.1f}s")

    torchaudio.save(args.output_wav, wav.cpu(), 16000)
